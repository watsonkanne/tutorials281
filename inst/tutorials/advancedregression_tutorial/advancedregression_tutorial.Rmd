...
$$ Y_i = \hat{\alpha} + \hat{\beta}_1 X_{1i} + \hat{\beta}_2 X_{2i} + ... + \hat{\beta}_n X_{ni} + \hat{\epsilon}_i $$

As you can see above, much of the model remains the same. There is still only one set of values for our dependent variable ($Y_i$), one intercept ($\hat{\alpha}$), and one set of residuals ($\hat{\epsilon}_i$). 
...
$$ Y_i = \hat{\alpha} + \hat{\beta}_1 X_{1i} + \hat{\beta}_2 X_{2i} + \hat{\epsilon}_i $$
...
The right-hand side of the equation above is the same as our standard multiple regression model. The change appears on the left-hand side, where instead of $Y$, we have $P(Y_i = 1|X_{ni})$, the probability that $Y$ equals one, given our values of X.
